/** Best-effort secret redaction for transcript-derived text before it is embedded in the HTML report. */
import type { AnalyzedSession } from "./types.ts";
...
/**
 * Rewrite every untrusted, transcript-derived string field on the session in place,
 * preserving object references shared by `biggestItems` / the loaded filter and
 * leaving precomputed numeric `tokensEst` untouched. Returns the same session for chaining.
 */
export const redactSession = (a: AnalyzedSession): AnalyzedSession => {
  for (const e of a.events) {
    if (e.title) e.title = redactText(e.title);
    if (e.preview) e.preview = redactText(e.preview);
    if (e.body) e.body = redactText(e.body);
  }
  for (const s of a.subagents) {
    if (s.description) s.description = redactText(s.description);
  }
  if (a.title) a.title = redactText(a.title);
  return a;
};
